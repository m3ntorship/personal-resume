...
const WorkProcess = () => {
  ...
  if (workProcessData) {
    ...
    return (
      <section className="border-4 border-c1000 lg:pt-24 pt-16 pb-32 bg-c400 lg:rounded-tl-md lg:mt-24 mx-auto">
        <Heading
          as="h1"
          fontSize={HEADING_OPTIONS.FONT_SIZES.MEDIUM}
          color={HEADING_OPTIONS.COLOR.PRIMARY}
          className="text-center lg:mb-16 mb-16"
        >
          {title}
        </Heading>
        <div className="container relative">
          <div className=" grid  lg:grid-cols-4 sm:grid-cols-1 items-center justify-center ">
            {circleItem.map(item => {
              const { icon, title, id } = item;
              return (
                <div className="">
                  <div className="relative z-50 mx-auto lg:mb-0 mb-12" key={id}>
                    <div className="mx-auto circle__container flex justify-center items-center lg:h-40 lg:w-40 h-70 w-70 rounded-full parent-circle bg-white hover:bg-c100">
          ...
          <div
            ...
            className="lg:h-2 lg:w-4/5 absolute z-0 bg-c100 opacity-25"
          ></div>
        </div>
      </section>
    );
